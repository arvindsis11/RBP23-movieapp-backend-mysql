...
//	}
	...
	void testSearchMovieById() {
		...
		movie.setMovieName("endgame");
		movie.setTotalTickets(100);
//		when(movieRepo.findByMovieId(anyString())).thenReturn(Optional.of(movie)); for mongo
//		ResponseEntity<?> response = movieService.searchMovieById(anyString());
		when(movieRepo.findByMovieId(anyLong())).thenReturn(Optional.of(movie));
		ResponseEntity<?> response = movieService.searchMovieById(anyLong());
	...
	void testDeleteMovie() {
		...
		movie.setTotalTickets(100);
		when(movieRepo.findByMovieId(anyLong())).thenReturn(Optional.of(movie));
		ResponseEntity<?> response = movieService.deleteMovie(anyLong());
